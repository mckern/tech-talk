...
function _classCallCheck(instance, Constructor) {
	...
	if ((typeof exports === 'undefined' ? 'undefined' : _typeof2(exports)) === 'object' && (typeof module === 'undefined' ? 'undefined' : _typeof2(module)) === 'object') module.exports = factory();
	else if (typeof define === 'function' && define.amd) define([], factory);
	else if ((typeof exports === 'undefined' ? 'undefined' : _typeof2(exports)) === 'object') exports["io"] = factory();
	...
	return ( /******/ function (modules) { // webpackBootstrap
	...
	}, /* 4 */ /***/ function (module, exports) { // shim for using process in browser
		...
		function debug(namespace) { // define the `disabled` version
			...
			function enabled() {
				...
				var index = 0;
				args[0] = args[0].replace(/%([a-z%])/g, function (match, format) { // if we encounter an escaped % then don't increase the array index
		...
		(function (module, global) {
			...
			var define = false; /*! JSON v3.3.2 | http://bestiejs.github.io/json3 | Copyright 2012-2014, Kit Cambridge | http://kit.mit-license.org */ ;
			(function () { // Detect the `define` function exposed by asynchronous module loaders. The
				...
				function runInContext(context, exports) {
					...
					var objectProto = Object.prototype,
						...
						_isProperty, _forEach, undef; // Test the `Date#getUTC*` methods. Based on work by @Yaffle.
					var isExtended = new Date(-3509827334573292);
					try { // The `getUTCFullYear`, `Month`, and `Date` methods return nonsensical
					...
					function has(name) {
						...
						var isSupported;
						if (name == "bug-string-char-index") { // IE <= 7 doesn't support accessing string characters using square
							// bracket notation. IE 8 only supports this for primitives.
							isSupported = "a" [0] != "a";
						} else if (name == "json") { // Indicates whether both `JSON.stringify` and `JSON.parse` are
							// supported.
							...
							if (name == "json-stringify") {
								...
								if (stringifySupported) { // A test function object with a custom `toJSON` method.
							...
							if (name == "json-parse") {
								...
								if (typeof parse == "function") {
									try { // FF 3.1b1, b2 will throw an exception if a bare literal is provided.
										...
										if (parse("0") === 0 && !parse(false)) { // Simple parsing test.
											...
											if (parseSupported) {
												try { // Safari <= 5.1.2 and FF 3.1b1 allow unescaped tabs in strings.
													parseSupported = !parse('"\t"');
												} catch (exception) {}
												if (parseSupported) {
													try { // FF 4.0 and 4.0.1 allow leading `+` signs and leading
												...
												if (parseSupported) {
													try { // FF 4.0, 4.0.1, and Rhino 1.7R3-R4 allow trailing decimal
					...
					if (!has("json")) { // Common `[[Class]]` name aliases.
						...
						if (!(_isProperty = objectProto.hasOwnProperty)) {
							_isProperty = function isProperty(property) {
								var members = {},
									constructor;
								if ((members.__proto__ = null, members.__proto__ = { // The *proto* property cannot be set multiple times in recent
										// versions of Firefox and SeaMonkey.
										"toString": 1
									}, members).toString != getClass) { // Safari <= 2.0.3 doesn't implement `Object#hasOwnProperty`, but
								...
								} else { // Capture a reference to the top-level `Object` constructor.
						...
						_forEach = function forEach(object, callback) {
							var size = 0,
								Properties, members, property; // Tests for bugs in the current environment's `for...in` algorithm. The
							...
							} else if (size == 2) { // Safari <= 2.0.4 enumerates shadowed properties twice.
							...
							} else { // No bugs detected; use the standard `for...in` algorithm.
								_forEach = function forEach(object, callback) {
									var isFunction = getClass.call(object) == functionClass,
						...
						if (!has("json-stringify")) { // Internal: A map of control characters and their escaped equivalents.
							...
							var toPaddedString = function toPaddedString(width, value) { // The `|| 0` expression is necessary to work around a bug in
							...
							var serialize = function serialize(property, object, callback, properties, whitespace, indentation, stack) {
								...
								try { // Necessary for host object support.
									value = object[property];
								} catch (exception) {}
								if ((typeof value === 'undefined' ? 'undefined' : _typeof2(value)) == "object" && value) {
									className = getClass.call(value);
									if (className == dateClass && !_isProperty.call(value, "toJSON")) {
										if (value > -1 / 0 && value < 1 / 0) { // Dates are serialized according to the `Date#toJSON` method
									...
									} else if (typeof value.toJSON == "function" && (className != numberClass && className != stringClass && className != arrayClass || _isProperty.call(value, "toJSON"))) { // Prototype <= 1.6.1 adds non-standard `toJSON` methods to the
								...
								className = getClass.call(value);
								if (className == booleanClass) { // Booleans are represented literally.
									return "" + value;
								} else if (className == numberClass) { // JSON numbers must be finite. `Infinity` and `NaN` are serialized as
									// `"null"`.
									return value > -1 / 0 && value < 1 / 0 ? "" + value : "null";
								} else if (className == stringClass) { // Strings are double-quoted and escaped.
									return quote("" + value);
								...
								if ((typeof value === 'undefined' ? 'undefined' : _typeof2(value)) == "object") { // Check for cyclic structures. This is a linear search; performance
									...
									for (length = stack.length; length--;) {
										if (stack[length] === value) { // Cyclic structures cannot be serialized by `JSON.stringify`.
											throw TypeError();
										}
									...
									if (className == arrayClass) { // Recursively serialize array elements.
										...
										_forEach(properties || value, function (property) {
											...
											if (element !== undef) { // According to ES 5.1 section 15.12.3: "If `gap` {whitespace}
							...
							exports.stringify = function (source, filter, width) {
								...
								if (objectTypes[typeof filter === 'undefined' ? 'undefined' : _typeof2(filter)] && filter) {
									if ((className = getClass.call(filter)) == functionClass) {
										callback = filter;
									} else if (className == arrayClass) { // Convert the property names array into a makeshift set.
										properties = {};
										for (var index = 0, length = filter.length, value; index < length; value = filter[index++], (className = getClass.call(value), className == stringClass || className == numberClass) && (properties[value] = 1)) {}
									}
								}
								if (width) {
									if ((className = getClass.call(width)) == numberClass) { // Convert the `width` to an integer and create a string containing
						...
						if (!has("json-parse")) {
							...
							var lex = function lex() {
								...
								while (Index < length) {
									...
									switch (charCode) {
										...
										case 10:
										case 13:
										case 32: // Skip whitespace tokens, including tabs, carriage returns, line
										...
										case 93:
										case 58:
										case 44: // Parse a punctuator token (`{`, `}`, `[`, `]`, `:`, or `,`) at
											// the current position.
											value = charIndexBuggy ? source.charAt(Index) : source[Index];
											Index++;
											return value;
										case 34: // `"` delimits a JSON string; advance to the next character and
											// begin parsing the string. String tokens are prefixed with the
											// sentinel `@` character to distinguish them from punctuators and
											// end-of-string tokens.
											for (value = "@", Index++; Index < length;) {
												charCode = source.charCodeAt(Index);
												if (charCode < 32) { // Unescaped ASCII control characters (those with a code unit
													// less than the space character) are not permitted.
													abort();
												} else if (charCode == 92) { // A reverse solidus (`\`) marks the beginning of an escaped
													...
													switch (charCode) {
														...
														case 110:
														case 102:
														case 114: // Revive escaped control characters.
															value += Unescapes[charCode];
															Index++;
															break;
														case 117: // `\u` marks the beginning of a Unicode escape sequence.
													...
													if (charCode == 34) { // An unescaped double-quote character marks the end of the
														// string.
														break;
													}
													charCode = source.charCodeAt(Index);
											...
											if (source.charCodeAt(Index) == 34) { // Advance to the next character and return the revived string.
												Index++;
												return value;
											} // Unterminated string.
											abort();
										default: // Parse numbers and literals.
											...
											if (charCode >= 48 && charCode <= 57) { // Leading zeroes are interpreted as octal literals.
												...
												if (source.charCodeAt(Index) == 46) {
													...
													if (position == Index) { // Illegal trailing decimal.
														abort();
													}
													Index = position;
												...
												if (charCode == 101 || charCode == 69) {
													...
													if (position == Index) { // Illegal empty exponent.
														abort();
													}
													Index = position;
									...
									if (value == "[") { // Parses a JSON array, returning a new JavaScript array.
										...
										for (;; hasMembers || (hasMembers = true)) {
											...
											if (hasMembers) {
												if (value == ",") {
													value = lex();
													if (value == "]") { // Unexpected trailing `,` in array literal.
														abort();
													}
												} else { // A `,` must separate each array element.
													abort();
												}
										...
										return results;
									} else if (value == "{") { // Parses a JSON object, returning a new JavaScript object.
										results = {};
										for (;; hasMembers || (hasMembers = true)) {
											...
											if (hasMembers) {
												if (value == ",") {
													value = lex();
													if (value == "}") { // Unexpected trailing `,` in object literal.
														abort();
													}
												} else { // A `,` must separate each object member.
													abort();
												}
							...
							var walk = function walk(source, property, callback) {
								var value = source[property],
									length;
								if ((typeof value === 'undefined' ? 'undefined' : _typeof2(value)) == "object" && value) { // `forEach` can't be used to traverse an array in Opera <= 8.54
				...
				if (freeExports && !isLoader) { // Export for CommonJS environments.
					runInContext(root, freeExports);
				} else { // Export for web browsers and JavaScript engines.
					var nativeJSON = root.JSON,
						previousJSON = root["JSON3"],
						isRestored = false;
					var JSON3 = runInContext(root, root["JSON3"] = { // Public: Restores the original value of the global `JSON` object and
		...
		(function (global) {
			...
			exports.removeBlobs = function (data, callback) {
				function _removeBlobs(obj, curKey, containingObject) {
					...
					} else if (obj && 'object' == (typeof obj === 'undefined' ? 'undefined' : _typeof2(obj)) && !isBuf(obj)) { // and object
						for (var key in obj) {
		...
		(function (global) {
			...
			Socket.prototype.open = function () {
				...
				if (this.rememberUpgrade && Socket.priorWebsocketSuccess && this.transports.indexOf('websocket') !== -1) {
					transport = 'websocket';
				} else if (0 === this.transports.length) { // Emit error on next tick so it can be listened to
					var self = this;
					setTimeout(function () {
		...
		(function (global) {
			...
			Request.prototype.create = function () {
				...
				try {
					...
					if (this.supportsBinary) { // This has to be done after open because Firefox is stupid
					...
					xhr.send(this.data);
				} catch (e) { // Need to defer since .create() is called directly fhrom the constructor
		...
		Polling.prototype.onData = function (data) {
			...
			var callback = function callback(packet, index, total) { // if its the first message we consider the transport open
		...
		(function (global) {
			...
			exports.encodeBase64Packet = function (packet, callback) {
				...
				} catch (e) { // iPhone Safari doesn't let you apply with typed arrays
			...
			exports.decodePayloadAsBinary = function (data, binaryType, callback) {
				...
				while (bufferTail.byteLength > 0) {
					...
					if (isString) {
						...
						} catch (e) { // iPhone Safari doesn't let you apply to typed arrays
		...
		(function (global) {
			...
			function hasBinary(data) {
				function _hasBinary(obj) {
					...
					} else if (obj && 'object' == (typeof obj === 'undefined' ? 'undefined' : _typeof2(obj))) { // see: https://github.com/Automattic/has-binary/pull/4
		...
		(function (module, global) {
			...
			(function (root) { // Detect free variables `exports`
				...
				for (var i = 0, l = total; i < l; i++) {
					(function (packet) {
						parser.encodePacket(packet, self.supportsBinary, function (data) {
							if (!self.usingBrowserWebSocket) { // always create a new object (GH-437)
								var opts = {};
								if (packet.options) {
							...
							// throw an error
							try {
								if (self.usingBrowserWebSocket) { // TypeError is thrown when passing the second argument on Safari
									self.ws.send(data);
								} else {
		...
		/* (ignored) */ /***/ }, /* 42 */ /***/ function (module, exports) {
		var indexOf = [].indexOf;
		...
		Socket.prototype.ack = function (id) {
			var self = this;
			var sent = false;
			return function () { // prevent double callbacks
				if (sent) return;
				sent = true;
				var args = toArray(arguments);
		...
		Socket.prototype.destroy = function () {
			if (this.subs) { // clean subscriptions to avoid reconnections
...
lib.ensureRuntimeDependencies_ = function () {
	...
	for (var path in lib.runtimeDependencies_) {
		...
		var names = path.split('.'); // In a document context 'window' is the global object.  In a worker it's
		// called 'self'.
		var obj = window || self;
...
lib.f.alarm = function (callback, opt_ms) {
	...
	return function () { // This outer function is called immediately.  It's here to capture a new
...
lib.PreferenceManager.Record.prototype.get = function () {
	if (this.currentValue === this.DEFAULT_VALUE) {
		if (/^(string|number)$/.test(_typeof2(this.defaultValue))) return this.defaultValue;
		if (_typeof2(this.defaultValue) == 'object') { // We want to return a COPY of the default value so that users can
...
lib.PreferenceManager.prototype.diff = function (a, b) { // If the types are different, or the type is not a simple primitive one.
	if ((typeof a === 'undefined' ? 'undefined' : _typeof2(a)) !== (typeof b === 'undefined' ? 'undefined' : _typeof2(b)) || !/^(undefined|boolean|number|string)$/.test(typeof a === 'undefined' ? 'undefined' : _typeof2(a))) {
		return true;
	}
	return a !== b;
};
...
lib.Storage.Chrome.prototype.setItem = function (key, value, opt_callback) {
	...
	var onComplete = function onComplete() {
		if (chrome.runtime.lastError) { // Doesn't seem to be any better way of handling this.
			// https://crbug.com/764759
			if (chrome.runtime.lastError.message.indexOf('MAX_WRITE_OPERATIONS')) {
				console.warn('Will retry save of ' + key + ' after exceeding quota', chrome.runtime.lastError);
				setTimeout(function () {
					return _this2.setItem(key, value, onComplete);
				}, 1000);
				return;
			} else {
...
lib.TestManager.Log = function () {
	...
	this.data = '';
	this.prefix_ = '';
	this.prefixStack_ = 0; // Capture all the console entry points in case code at runtime calls these
	...
	['log', 'debug', 'info', 'warn', 'error'].forEach(function (level) {
		...
		var oLog = _this3.console_[level];
		_this3[level] = _this3.console_[level] = function () {
			for (var _len2 = arguments.length, args = Array(_len2), _key2 = 0; _key2 < _len2; _key2++) {
				args[_key2] = arguments[_key2];
			}
			if (_this3.save) _this3.data += _this3.prefix_ + msgPrefix + args.join(' ') + '\n';
			oLog.apply(_this3.console_, args);
		};
	}); // Wrap/bind the group functions.
	['group', 'groupCollapsed'].forEach(function (group) {
		var oGroup = _this3.console_[group];
		_this3[group] = _this3.console_[group] = function () {
			var label = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
			oGroup(label);
			if (_this3.save) _this3.data += _this3.prefix_ + label + '\n';
			_this3.prefix_ = '  '.repeat(++_this3.prefixStack_);
		};
	});
...
lib.TestManager.TestRun.prototype.onUncaughtException_ = function (message, file, line) {
	...
	if (message == 'Uncaught ' + this.currentResult.expectedErrorMessage_) { // Test cases may need to raise an unhandled exception as part of the test.
		return;
	}
	var when = 'during';
...
lib.TestManager.TestRun.prototype.onResultComplete = function (result) {
	...
	else if (result.duration > 500) this.log.warn('Slow test took ' + this.msToSeconds_(result.duration));
	this.log.groupEnd();
...
lib.TestManager.TestRun.prototype.runNextTest_ = function () {
	...
	} catch (ex) { // If test suite setup fails we're not even going to try to run the tests.
	...
	} catch (ex) { // Result.run() should catch test exceptions and turn them into failures.
...
lib.TestManager.Result.prototype.assertEQ = function (actual, expected, opt_name) { // Utility function to pretty up the log.
	function format(value) {
		...
		if (ary.length > 1) { // If the string has newlines, start it off on its own line so that
...
lib.encodeUTF8 = function (str) {
	var ret = '';
	for (var i = 0; i < str.length; i++) { // Get a unicode code point out of str.
		var c = str.charCodeAt(i);
		if (0xDC00 <= c && c <= 0xDFFF) {
			...
			if (i + 1 < str.length) {
				...
				if (0xDC00 <= d && d <= 0xDFFF) { // Swallow a surrogate pair.
...
lib.resource.add('libdot/changelog/version', 'text/plain', '1.19' + '');
lib.resource.add('libdot/changelog/date', 'text/plain', '2017-10-16' + ''); // SOURCE FILE: hterm/js/hterm.js
// Copyright (c) 2012 The Chromium OS Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.
'use strict';
...
hterm.copySelectionToClipboard = function (document) {
	...
	} catch (firefoxException) { // Ignore this. FF throws an exception if there was an error, even though
...
hterm.Keyboard.prototype.onKeyPress_ = function (e) {
	...
	if ((e.ctrlKey || e.metaKey) && (lowerKey == 'c' || lowerKey == 'v')) { // On FF the key press (not key down) event gets fired for copy/paste.
...
hterm.Keyboard.prototype.onKeyDown_ = function (e) {
	...
	if (action.substr(0, 2) == '\x1b[' && (alt || control || shift || meta)) { // The action is an escape sequence that and it was triggered in the
...
hterm.PreferenceManager.defaultPreferences = {
	...
	'enable-clipboard-write': [hterm.PreferenceManager.categories.CopyPaste, true, 'bool', 'Allow the host to write directly to the system clipboard.'],
	...
	'font-size': [hterm.PreferenceManager.categories.Appearance, 32, 'int', 'The default font size in pixels.'],
	...
	'max-string-sequence': [hterm.PreferenceManager.categories.Encoding, 100000, 'int', 'Max length of a DCS, OSC, PM, or APS sequence before we give up and ' + 'ignore the code.'],
...
hterm.Screen.prototype.insertString = function (str) {
	...
	if (reverseOffset < 0) { // A negative reverse offset means the cursor is positioned past the end
		...
		if (!(this.textAttributes.underline || this.textAttributes.strikethrough || this.textAttributes.background || this.textAttributes.wcNode || !this.textAttributes.asciiNode || this.textAttributes.tileData != null)) { // Best case scenario, we can just pretend the spaces were part of the
			// original string.
			str = ws + str;
		} else if (cursorNode.nodeType == Node.TEXT_NODE || !(cursorNode.wcNode || !cursorNode.asciiNode || cursorNode.tileNode || cursorNode.style.textDecoration || cursorNode.style.backgroundColor)) { // Second best case, the current node is able to hold the whitespace.
...
hterm.Screen.prototype.expandSelection = function (selection) {
	...
	if (endPosition == -1) return; // Use the user configurable match settings.
	var leftMatch = this.wordBreakMatchLeft;
	var rightMatch = this.wordBreakMatchRight;
	var insideMatch = this.wordBreakMatchMiddle; //Move start to the left.
...
hterm.ScrollPort.prototype.decorate = function (div) {
	...
	this.iframe_.style.cssText = 'border: 0;' + 'height: 100%;' + 'position: absolute;' + 'width: 100%'; // Set the iframe src to # in FF.  Otherwise when the frame's
	...
	doc.body.addEventListener('keydown', this.onBodyKeyDown_.bind(this)); // This is the main container for the fixed rows.
	...
	this.screen_.appendChild(this.rowNodes_); // Two nodes to hold offscreen text during the copy event.
...
hterm.ScrollPort.prototype.onDragAndDrop_ = function (e) {
	e.preventDefault();
	var data = void 0;
	var format = void 0; // If the shift key isn't active, try to find a text source (but not plain
...
hterm.Terminal = function (opt_profileId) {
	...
	this.bellAudio_.setAttribute('preload', 'auto'); // All terminal bell notifications that have been generated (not necessarily
	// shown).
...
hterm.Terminal.prototype.setProfile = function (profileId, opt_callback) {
	...
	this.prefs_.addObservers(null, {
		...
		'max-string-sequence': function maxStringSequence(v) {
		...
		'pass-alt-number': function passAltNumber(v) {
			if (v == null) {
				var osx = window.navigator.userAgent.match(/Mac OS X/); // Let Alt-1..9 pass to the browser (to control tab switching) on
		...
		'pass-ctrl-number': function passCtrlNumber(v) {
			if (v == null) {
				var osx = window.navigator.userAgent.match(/Mac OS X/); // Let Ctrl-1..9 pass to the browser (to control tab switching) on
				// non-OS X systems, or if hterm is not opened in an app window.
				v = !osx && hterm.windowType != 'popup';
			}
		...
		'pass-meta-number': function passMetaNumber(v) {
			if (v == null) {
				var osx = window.navigator.userAgent.match(/Mac OS X/); // Let Meta-1..9 pass to the browser (to control tab switching) on
...
hterm.Terminal.prototype.runCommandClass = function (commandClass, argString) {
	...
	if ((typeof environment === 'undefined' ? 'undefined' : _typeof2(environment)) != 'object' || environment == null) environment = {};
	var self = this;
...
hterm.Terminal.prototype.softReset = function () { // Reset terminal options to their default values.
	...
	this.alternateScreen_.textAttributes.resetColorPalette(); // The xterm man page explicitly says this will happen on soft reset.
...
hterm.Terminal.prototype.showOverlay = function (msg, opt_timeout) {
	...
	this.overlayTimeout_ = setTimeout(function () {
		_this4.overlayNode_.style.opacity = '0';
		_this4.overlayTimeout_ = setTimeout(function () {
...
hterm.Terminal.prototype.copyStringToClipboard = function (str) {
	...
	hterm.copySelectionToClipboard(this.document_); // IE doesn't support selection.extend. This means that the selection
	// won't return on IE.
	if (selection.extend) {
...
hterm.Terminal.prototype.openUrl = function (url) {
	if (window.chrome && window.chrome.browser) { // For Chrome v2 apps, we need to use this API to properly open windows.
...
hterm.Terminal.prototype.onMouse_ = function (e) {
	...
	e.processedByTerminalHandler_ = true; // One based row/column stored on the mouse event.
...
hterm.TextAttributes = function (document) {
	...
	this.backgroundSource = this.SRC_DEFAULT; // These properties cache the value in the color table, but foregroundSource
...
hterm.TextAttributes.prototype.isDefault = function () {
	return this.foregroundSource == this.SRC_DEFAULT && this.backgroundSource == this.SRC_DEFAULT && !this.bold && !this.faint && !this.italic && !this.blink && !this.underline && !this.strikethrough && !this.inverse && !this.invisible && !this.wcNode && this.asciiNode && this.tileData == null;
};
/**
...
hterm.TextAttributes.prototype.syncColors = function () {
	...
	if (this.inverse) {
		...
		backgroundSource = this.foregroundSource; // We can't inherit the container's color anymore.
...
hterm.VT = function (terminal) {
	...
	this.mouseReport = this.MOUSE_REPORT_DISABLED; // Parse state left over from the last parse.  You should use the parseState
	...
	this.updateEncodingState_(); // Saved state used in DECSC.
	//
...
hterm.VT.ParseState.prototype.iarg = function (argnum, defaultValue) {
	...
	if (str) {
		var ret = parseInt(str, 10); // An argument of zero is treated as the default value.
...
hterm.VT.prototype.onTerminalMouse_ = function (e) {
	if (this.mouseReport == this.MOUSE_REPORT_DISABLED) return; // Temporary storage for our response.
	var response; // Modifier key state.
	var mod = 0;
	if (e.shiftKey) mod |= 4;
	if (e.metaKey || this.terminal.keyboard.altIsMeta && e.altKey) mod |= 8;
	if (e.ctrlKey) mod |= 16; // TODO(rginda): We should also support mode 1005 and/or 1006 to extend the
	...
	switch (e.type) {
		case 'wheel': // Mouse wheel is treated as button 1 or 2 plus an additional 64.
			...
			b |= mod;
			response = '\x1b[M' + String.fromCharCode(b) + x + y; // Keep the terminal from scrolling.
			e.preventDefault();
			break;
		case 'mousedown': // Buttons are encoded as button number plus 32.
			var b = Math.min(e.button, 2) + 32; // And mix in the modifier keys.
			b |= mod;
			response = '\x1b[M' + String.fromCharCode(b) + x + y;
			break;
		case 'mouseup': // Mouse up has no indication of which button was released.
...
hterm.VT.prototype.updateEncodingState_ = function () {
	var _this5 = this; // If we're in UTF8 mode, don't suport 8-bit escape sequences as we'll never
...
hterm.VT.prototype.parseCSI_ = function (parseState) {
	...
	} else if (ch >= '0' && ch <= '9') { // Next byte in the current parameter.
	...
	} else if (ch >= ' ' && ch <= '?' && ch != ':') { // Modifier character.
		if (!args.length) {
...
hterm.VT.prototype.parseUntilStringTerminator_ = function (parseState) {
	...
	var foundTerminator = void 0; // If the next escape we see is not a start of a ST, fall through.  This will
	...
	if (!foundTerminator) { // No terminator here, have to wait for the next string.
		args[0] += buf;
		var abortReason;
		if (args[0].length > this.maxStringSequence) abortReason = 'too long: ' + args[0].length; // Special case: If our buffering happens to split the ST (\e\\), we have to
	...
	if (args[0].length + nextTerminator > this.maxStringSequence) { // We found the end of the sequence, but we still think it's too long.
...
hterm.VT.CSI['J'] = hterm.VT.CSI['?J'] = function (parseState, code) {
	...
	if (!arg || arg == 0) {
		...
		this.terminal.clear();
	} else if (arg == 3) { // The xterm docs say this means "Erase saved lines", but we'll just clear
...
hterm.VT.CSI['m'] = function (parseState) {
	...
	for (var i = 0; i < parseState.args.length; i++) {
		...
		if (arg < 30) {
			...
			} else if (arg == 48) { // First check for true color definition
				var trueColor = getTrueColor(i);
				if (trueColor != null) {
...
hterm.VT.CharacterMap.prototype.sync_ = function (opt_glmap) {
	var _this6 = this; // If there are no maps, then reset the state back.
...
lib.resource.add('hterm/concat/date', 'text/plain', 'Sun, 22 Oct 2017 11:00:51 +0000' + '');
lib.resource.add('hterm/changelog/version', 'text/plain', '1.73' + '');
lib.resource.add('hterm/changelog/date', 'text/plain', '2017-10-16' + '');
...
var Wetty = function () {
	...
	_createClass(Wetty, [{
		...
		key: 'onTerminalResize',
		value: function onTerminalResize(col, row) {
			socket.emit('resize', {
				col: col,
				row: row
			});
